...
# Putnum County Number for stateplane library
countyfips = 47141
...
lats = [36.1621275, 36.1623484, 36.1624610, 36.1622510,	36.1621370]
lons = [-85.5423456,-85.5425870,-85.5424020,-85.5421767,-85.5423339]
...
GPIO.output(22, GPIO.HIGH)
time.sleep(1)
pwmright.ChangeDutyCycle(rformid)
pwmleft.ChangeDutyCycle(lformid)
time.sleep(3)
...
for i in range(0,len(lats)):
    ...
    while (waydistance > 0.6):
        ...
        if (waydist2 > waydist1):
            ...
            if (error2 > error1):
                if (quad == 2) or (quad == 3):
                    left = lformid - (scale*lformid) + ((dtheta/100)*lformid)
                    right = rformid + (scale*rformin) + ((dtheta/100)*rformid)
